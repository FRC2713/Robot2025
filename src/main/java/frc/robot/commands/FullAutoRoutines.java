...
public class FullAutoRoutines {
    ...
    private static Command resetToInitialPose(Drivetrain drive, AutoRoutine routine, AutoTrajectory initialTrajectory) {
        return Commands.runOnce(
            () -> initialTrajectory.getInitialPose().ifPresentOrElse(pose -> drive.resetOdometry(pose), routine::kill), drive);
    }
    ...
    public static Command simplePathAuto(Drivetrain drive, AutoFactory factory) {
        ...
        routine.active().onTrue(
            Commands.sequence(
                resetToInitialPose(drive, routine, simplePath),
                simplePath.cmd()
            )
        );
