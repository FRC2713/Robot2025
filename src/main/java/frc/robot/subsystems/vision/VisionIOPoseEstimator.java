  ...
  public void update() {
    ...
    if (poseArray.length > 0) {
      ...
      if (lastT != t) {
        ...
        Logger.recordOutput("Odometry/Latency", latency);
        ...
        if (latency < 0) {
          Logger.recordOutput("SLAMDunk/Problem", "Pose from the future!");
          Logger.recordOutput("Odometry/hasPose", false);
          return;
        }
        if (latency > 1) {
          Logger.recordOutput("SLAMDunk/Problem", "Latency > 1");
          Logger.recordOutput("Odometry/hasPose", false);
          return;
        }
        ...
        if ((pose2d
          ...
          Logger.recordOutput("Odometry/hasPose", false);
          Logger.recordOutput("SLAMDunk/Problem", "Jump protection");
          return;
        }

        if (Math.abs(pose.getTranslation().getZ()) > 0.1) {
          addingMeasurement = false;
          Logger.recordOutput("Odometry/hasPose", false);
          Logger.recordOutput("SLAMDunk/Problem", "Z > 0.1");
          return;
        }
        ...
        RobotContainer.driveSubsystem.addVisionMeasurement(
            ...
            new PoseEstimatorErrorStDevs(0.001, Units.degreesToRadians(9999)).toMatrix());

        Logger.recordOutput("Odometry/hasPose", true);
        Logger.recordOutput("SLAMDunk/Problem", "None");
