...
public class ClimberIOSim implements ClimberIO {
  ...
  private static final SingleJointedArmSim sim =
      new SingleJointedArmSim(
          ...
          ClimberConstants.kMinAngleRad,
          ClimberConstants.kMaxAngleRad,
          true,
          ClimberConstants.kInitialAngleRad);

  private PIDController pid = ClimberConstants.PID.createPIDController();

  public ClimberIOSim() {
    pid.enableContinuousInput(0, 2 * Math.PI);
  }

  private ArmFeedforward feedforward = ClimberConstants.PID.createArmFF();
  private double targetAngleDeg = Units.radiansToDegrees(ClimberConstants.kInitialAngleRad);
