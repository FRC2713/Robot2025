...
public class ClimberIOKrakens implements ClimberIO {
  ...
  public ClimberIOKrakens() {
    this.motor = new TalonFX(ClimberConstants.kCANId);
    config = ClimberConstants.createKrakenConfig();
    PhoenixUtil.tryUntilOk(5, () -> motor.getConfigurator().apply(config, 0.25));
    PhoenixUtil.tryUntilOk(
        5,
        () -> motor.setPosition(Units.radiansToRotations(ClimberConstants.kInitialAngleRad), 0.25));
  }
